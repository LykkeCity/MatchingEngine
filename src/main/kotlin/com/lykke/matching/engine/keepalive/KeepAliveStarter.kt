...
@Component
@Profile("default")
class KeepAliveStarter @Autowired constructor(private val config: Config, private val jedisHolder: JedisHolder) {
    @PostConstruct
    private fun start() {
        val generalHealthMonitor = GeneralHealthMonitor(listOf(jedisHolder))
        KeepAliveStarter.start(config.me.keepAlive, MeIsAliveResponseGetter(generalHealthMonitor), AppVersion.VERSION)
    }
}