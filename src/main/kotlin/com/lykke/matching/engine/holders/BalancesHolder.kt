...
class BalancesHolder(private val walletDatabaseAccessor: WalletDatabaseAccessor,
    ...
    private fun updateReservedBalance(clientId: String, assetId: String, timestamp: Date, balance: Double) {
        val client = balances.getOrPut(clientId) { HashMap() }
        val oldBalance = client[assetId]
        if (oldBalance == null) {
    ...
    fun updateReservedBalanceAndSend(id: String, type: String, timestamp: Date, clientId: String, assetId: String, newReservedBalance: Double, ignoreTrustedClients: Boolean = true) {
        if (ignoreTrustedClients && trustedClients.contains(clientId)) {
            return
        }
        val balance = getBalance(clientId, assetId)
        val reservedBalance = getReservedBalance(clientId, assetId)
        updateReservedBalance(clientId, assetId, timestamp, newReservedBalance)
        sendBalanceUpdate(BalanceUpdate(id, type, timestamp, listOf(ClientBalanceUpdate(clientId, assetId, balance, balance, reservedBalance, newReservedBalance))))
    }
