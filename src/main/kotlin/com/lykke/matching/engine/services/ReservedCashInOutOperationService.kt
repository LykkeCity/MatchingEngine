...
class ReservedCashInOutOperationService(private val assetsHolder: AssetsHolder,
    ...
    override fun processMessage(messageWrapper: MessageWrapper) {
        ...
        if (!updated) {
            ...
            LOGGER.info("Reserved cash in/out operation (${message.id}) for client ${message.clientId} asset ${message.assetId}, volume: ${RoundingUtils.roundForPrint(message.reservedVolume)}: unable to save balance")
            return
        }
        walletProcessor.apply().sendNotification(message.id, MessageType.RESERVED_CASH_IN_OUT_OPERATION.name)
