...
class CashTransferOperationService(private val balancesHolder: BalancesHolder,
    ...
    override fun processMessage(messageWrapper: MessageWrapper) {
        ...
        val toBalance = balancesHolder.getBalance(message.toClientId, message.assetId)
        ...
        balanceUpdateQueue.put(BalanceUpdate(message.id, MessageType.CASH_TRANSFER_OPERATION.name, Date(message.timestamp),
                listOf(ClientBalanceUpdate(message.fromClientId, message.assetId, fromBalance, balancesHolder.getBalance(message.fromClientId, message.assetId)),
                        ClientBalanceUpdate(message.toClientId, message.assetId, toBalance, balancesHolder.getBalance(message.toClientId, message.assetId)))))
