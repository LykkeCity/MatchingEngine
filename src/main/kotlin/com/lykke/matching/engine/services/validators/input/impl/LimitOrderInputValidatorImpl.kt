...
class LimitOrderInputValidatorImpl(val applicationSettingsHolder: ApplicationSettingsHolder) : LimitOrderInputValidator {
    ...
    private fun validateMaxValue(limitOrder: LimitOrder, assetPair: AssetPair) {
        ...
        if (limitOrder.getAbsVolume() * limitOrder.price > maxValue) {
            throw OrderValidationException(OrderStatus.InvalidValue, "value is too large (maxValue=$maxValue)")
        }
    }

    private fun validateStopOrderMaxValue(limitOrder: LimitOrder, assetPair: AssetPair) {
        val maxValue = assetPair.maxValue ?: return
        if ((limitOrder.lowerLimitPrice != null && limitOrder.getAbsVolume() * limitOrder.lowerPrice!! > maxValue
                        || limitOrder.upperLimitPrice != null && limitOrder.getAbsVolume() * limitOrder.upperPrice!! > maxValue)) {
            throw OrderValidationException(OrderStatus.InvalidValue, "value is too large (maxValue=$maxValue)")
        }
    }
