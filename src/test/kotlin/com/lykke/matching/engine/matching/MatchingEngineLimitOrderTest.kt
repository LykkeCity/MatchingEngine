...
class MatchingEngineLimitOrderTest : MatchingEngineTest() {
    ...
    @Test
    fun testMatchSellLimitOrderTradeAcrossWalletsLeadToNegativeSpread() {
        ...
        testOrderBookWrapper.addLimitOrder(buildLimitOrder(clientId = "Client3", price = 1.0, volume = 40.0, reservedVolume = 40.0))

        val limitOrder = buildLimitOrder(clientId = "Client2", price = 1.0, volume = -100.0)
        val matchingResult = match(limitOrder, getOrderBook("EURUSD", true))
        assertLimitOrderMatchingResult(matchingResult, status = OrderStatus.LeadToNegativeSpread, marketBalance = null, remainingVolume = BigDecimal.valueOf(-100.0))
        assertEquals(4, getOrderBook("EURUSD", true).size)
        assertEquals(0, getOrderBook("EURUSD", false).size)
    }

    @Test
    fun testMatchBuyLimitOrderTradeAcrossWalletsLeadToNegativeSpread() {
        testBalanceHolderWrapper.updateBalance("Client2", "USD", 200.0)
        testBalanceHolderWrapper.updateBalance("Client3", "EUR", 100.0)
        testBalanceHolderWrapper.updateBalance("Client1_1", "EUR", 100.0)
        testBalanceHolderWrapper.updateBalance("Client2_1", "EUR", 100.0)
        testBalanceHolderWrapper.updateBalance("Client1", "EUR", 100.0)

        testOrderBookWrapper.addLimitOrder(buildLimitOrder(clientId = "Client3", price = 1.0, volume = -40.0, reservedVolume = 40.0))
        testOrderBookWrapper.addLimitOrder(buildLimitOrder(clientId = "Client1_1", price = 1.1, volume = -40.0, reservedVolume = 44.0))
        testOrderBookWrapper.addLimitOrder(buildLimitOrder(clientId = "Client2_1", price = 1.2, volume = -40.0, reservedVolume = 48.0))
        testOrderBookWrapper.addLimitOrder(buildLimitOrder(clientId = "Client1", price = 1.3, volume = -40.0, reservedVolume = 52.0))
        ...
        assertLimitOrderMatchingResult(matchingResult, status = OrderStatus.LeadToNegativeSpread, marketBalance = null, remainingVolume = BigDecimal.valueOf(100.0))
        assertEquals(4, getOrderBook("EURUSD", false).size)
        assertEquals(0, getOrderBook("EURUSD", true).size)
    }
    ...
    fun testMatchLimitOrderSellFullBalance() {
        ...
        val matchingResult = match(buildLimitOrder(clientId = "Client1", assetId = "LKK1YLKK", volume = -5495.03, price = 1.0082,
