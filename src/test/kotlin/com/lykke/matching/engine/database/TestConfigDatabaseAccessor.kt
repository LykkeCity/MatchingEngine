...
class TestConfigDatabaseAccessor : ConfigDatabaseAccessor {
    private var settings = Settings(emptySet(), emptySet(), emptyMap())
    ...
    fun addMarketOrderPriceDeviationThreshold(assetPirId: String, threshold: BigDecimal) {
        ...
        settings = Settings(settings.trustedClients, settings.disabledAssets, thresholds)
    }

    fun clear() {
        settings = Settings(emptySet(), emptySet(), emptyMap())
    }
}